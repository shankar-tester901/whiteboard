function init(canvasCt) {
    var canvasCount = 0;

    if (canvasCt == 0) {
        canvasCount = 0;
    } else {
        canvasCount = canvasCt;
    }
    alert('init called here' + canvasCount);


    var canvas = document.createElement('canvas'); // creates new canvas element
    var ctx = canvas.getContext('2d');
    canvas.id = 'canvas-' + canvasCount; // gives canvas id
    alert(canvas.id);
    var channels_subscribe = [];
    channels_subscribe.push("text-" + canvas.id);
    channels_subscribe.push("image-" + canvas.id);
    ...
    pubnub.addListener({
        message: function(m) {
            ...
            if (m.subscribedChannel == channels_subscribe[0].toString()) {
                drawFromStream(m);
            } else {
    ...
    function postImageToSubscribedStream(message, canvas) {
        console.log('postImageToSubscribedStream ' + JSON.stringify(message.message));
        var img = new window.Image();
        img.addEventListener("load", function() {
            var ctx = canvas.getContext('2d');
            ctx.drawImage(img, 0, 0);
        });
        img.src = message.message.imageUrl;
    }
    ...
    function endDraw(e) {
        ...
        var publishConfig = {
            channel: channels_subscribe[0].toString(),
            message: data,
    ...
    // get the offset p+osition of the canvas
    var temp_canvasid = "#" + canvas.id
    var $canvas = $(temp_canvasid);
    // var $canvas = canvas.id;
    alert('Canvas id is ' + canvas.id);

    //  var Offset = $canvas.offset();
    var offsetX = 10; //Offset.left;
    var offsetY = 10; //Offset.top;
    ...
    function dragDrop(event, ui) {
        ...
        //  x = parseInt(ui.offset.left - 40) - 1;
        //  y = parseInt(ui.offset.top - 30);
        ...
        ctx.drawImage($tools[theIndex], 10, 20, width, height);
    ...
    function publishImage(data) {
        ...
        var message = {
            imageUrl: data.message //"https://cdn0.iconfinder.com/data/icons/basic-user-interface-elements/700/reload-refresh-arrow-repeat-2-512.png"
        }
        alert('Channel to Publish Image is ..  ' + channels_subscribe[1].toString());
        var publishConfig = {
            channel: channels_subscribe[1].toString(),
            message: message,
...
function newBoardFunc() {
    var count = Math.floor(Math.random() * (10 - 1 + 1)) + 1
    var canvasCount = count;
    init(canvasCount);
    ...
    // var deltax = canvasCount * 10;
    // var deltay = canvasCount * 10
    // alert(deltax);
    // canv.style.zIndex = canvasCount;
    // canv.style.position = "absolute";
    // canv.style.border = "2px solid";

    // canv.style.top = deltax;
    // canv.style.left = deltay;
